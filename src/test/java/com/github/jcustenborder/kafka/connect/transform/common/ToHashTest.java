...
public abstract class ToHashTest extends TransformationTest {
  protected ToHashTest(boolean isKey) {
    super(isKey);
  }
  ...
  public void bytes() throws UnsupportedEncodingException {
    ...
    SinkRecord outputRecord = this.transformation.apply(inputRecord);
    assertEquals(
        Hashing.sha1().hashString(expected, Charsets.UTF_8).toString(),
        outputRecord.value()
    );
    assertSchema(Schema.STRING_SCHEMA, outputRecord.valueSchema());
  }

  public static class ValueTest<R extends ConnectRecord<R>> extends ToHashTest {
    protected ValueTest() {
      super(false);
    }
